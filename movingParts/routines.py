...
REQUIRED_DCTS = ["metadata", "paths", "input_names", "default_params", "dependencies"]
...
def load_rtn_data(rtn_path):
    ...
    try:
        ...
        print(
            "!ERROR! the following keys are missing:",
        )
        print("        ", missing_keys)
            ...
            if line.startswith("<"):
                ...
                for arg_prvd in ln_data:
                    if arg_prvd == "":
                        continue
                    arg_nm = (
                        arg_prvd.replace(">", "").replace(" ", "").replace("\n", "")
                    )
...
class gnmAssembly:
    ...
    def submit_to_queue(
        ...
        cmd_lst = get_args_command_line(
            self, sample_obj, ref_gnm, depth, num_threads, min_len, adpt_fln
        )
